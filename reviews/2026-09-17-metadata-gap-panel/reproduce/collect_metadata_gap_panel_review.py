...
def main():
    ...
    correction = args.build / 'diagnostics/checker-first'
    diagnostic = json.loads((correction / 'report.json').read_text())
    assert not diagnostic['selected_checks_passed']
    assert diagnostic['checker_sha256'] == sha(correction / 'check_metadata_gap_panel_outputs.py')
    for source in correction.iterdir():
        copy(source, 'checker-correction/' + source.name)
