...
def verify_preview_paragraphs(document,preview):
    compact=lambda s:''.join(s.split())
    counts=Counter(compact(Paragraph(p,document).text) for node in body(document) for p in node.iter(qn('w:p')) if compact(Paragraph(p,document).text))
    pages=pdf_raw_page_texts(preview);text=''.join(pages)
    ...
    try:
        for case in a.cases:
            ...
            for language in ['english','chinese']:
                ...
                row['word_preview_paragraphs_checked']=verify_preview_paragraphs(after_doc,preview)
                if case=='partial':
