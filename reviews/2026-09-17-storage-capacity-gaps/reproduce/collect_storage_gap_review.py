"""Archive verified same-fixture 0.3.31 → 0.3.32 bilingual evidence."""
import argparse
import json
...
from check_storage_gap_outputs import CASES
...
def main():
    ...
    report=json.loads((a.build/'storage-gap-report.json').read_text())
    ...
    assert sha(ROOT/'scripts/check_storage_gap_outputs.py')==report['checker_sha256']
    assert sha(a.english/'scripts/storage_gap_contract.py')==report['contract_sha256']
    ...
    for root in [a.prior,a.build]:
        ...
        assert renders['all_renders_succeeded'] and len(renders['renders'])==36 and all(r['rendered'] for r in renders['renders'])
        ...
        preview=json.loads((root/'word-preview-storage-missing-storage-partial-storage-zero-storage-complete-empty-negative.json').read_text())
    ...
    assert json.loads((a.candidate/'storage-gap-scope.json').read_text())['passed']
    ...
    for prefix,root in [('before',a.prior),('after',a.build)]:
        ...
        for name in ['manifest.json','missing-info-render-report.json','owned-test-template-cleanup.json',
                     'word-preview-storage-missing-storage-partial-storage-zero-storage-complete-empty-negative.json']:copy(root/name,prefix+'/'+name)
    for locale in ['en','zh-Hant']:
        for case in CASES:
            for suffix in ['.json','.events.json']:copy(a.english/'fixtures/pilot'/locale/(case+suffix),'fixtures/'+locale+'/'+case+suffix)
    for name in ['storage-gap-report.json','worker-start.json','worker-lifecycle.json','runtime-restoration.json']:
    ...
    for name in ['storage-gap-scope.json','format-translation-probe.json','personal-data-translation-proof.json','missing-info-translation-proof.json']:
        copy(a.candidate/name,'probes/'+name)
    for name in sorted(set(report['helper_sha256'])|{'check_storage_gap_outputs.py','collect_storage_gap_review.py','probe_storage_gap_scope.py','capture_storage_pages.py'}):
        copy(ROOT/'scripts'/name,'reproduce/'+name)
    for name in ['storage_gap_contract.py','generate_storage_gap_fixtures.py']:copy(a.english/'scripts'/name,'reproduce/'+name)
    copy(ROOT/'pipeline.yml','reproduce/pipeline.yml')
    copy(ROOT/'docs/storage-gap-translation-delta.json','reproduce/storage-gap-translation-delta.json')
    ...
    print(json.dumps({'archived':str(a.output),'candidate_native_outputs':36,'baseline_native_outputs':36,'word_previews':24,'files':len(checks)}))
