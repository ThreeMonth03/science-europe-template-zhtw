"""Q3-only 0.3.32 source/translation delta; retain Q2 and Q11 behavior gates."""
import argparse
import io
import json
...
from probe_personal_data_translation import verify_latest_translation_chain
...
def main():
    ...
    personal, following = verify_latest_translation_chain([pair(f.read_text()) for f in files])
    ...
    baseline = following['storage_gap']['baseline']
    ...
    prior = ROOT/'reviews/2026-09-17-format-reading/probes/format-reading-scope.json'
    ...
    for old_row in proof['rows']:
        ...
        assert differences == ['src/questions/03-docs-metadata.html.j2'], differences
        ...
        count = storage_checks(root, frozen, language)
        ...
        checks.append({'language': folder, 'before': before, 'after': after, 'changed': differences,
            'q3_exact_dom_checks': count, 'q2_exact_dom_checks': q2, 'q11_branch_checks': q11,
    ...
    report = {'passed': True, 'release_acceptance': False, 'rows': checks, 'reviewed_deltas': following,
        ...
        'contract_sha256': sha(a.english/'scripts/storage_gap_contract.py'),
        'package_sha256': {n: sha(a.build/n) for n in ['english.zip', 'chinese.zip']}}
    target = a.build/'storage-gap-scope.json'; assert not target.exists()
