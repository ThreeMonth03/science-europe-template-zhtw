...
def finish(english, tooling, output):
    ...
    assert Counter(v['sentence'] for v in preflight['blanks']) == Counter(TRANSLATIONS.keys())
    ...
    assert sum(before.values()) == 762 and sum(after.values()) == 766
    assert before - after == Counter({REMOVED: 1})
    assert after - before == Counter(TRANSLATIONS.items())
    assert sum((before & after).values()) == 761
    translation_proof = dict(before_units=762, after_units=766, retained_pairs=761,
        removed=[list(REMOVED)], added=[list(p) for p in TRANSLATIONS.items()],
    ...
    units = load_tree_manifest(reviewed)['units']; assert len(units) == 766
    ...
    for language, folder in [('english', 'en'), ('chinese', 'translated')]:
        ...
        with zipfile.ZipFile(package) as z:
            ...
            if language == 'english': assert not differences
    ...
    result = dict(passed=True, full_source_rehearsal=True, source_integrated=False, native_checked=False,
        ...
        translation_units=766, retained_translation_pairs=761, packages=packages,
