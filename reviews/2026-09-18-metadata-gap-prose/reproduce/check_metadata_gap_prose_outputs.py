...
def prose_geometry(before_bbox, after_bbox, before, after):
    ...
    metrics = {(round(line[1], 2), round(line[3], 2)) for prompt in old for line in prompt['lines']}
    assert metrics == {(round(line[1], 2), round(line[3], 2)) for line in new['lines']}, 'Warning font metrics or left alignment changed'
    ...
    try:
        for case in CASES:
            for language in ('english', 'chinese'):
                ...
                row['q3_missing_facts'] = [node['data-fact-id'] for node in after.select('#q-docs-metadata [data-fact-id][data-status="missing"]')]
                assert row['q3_missing_facts'] == [node['data-fact-id'] for node in before.select('#q-docs-metadata [data-fact-id][data-status="missing"]')]
