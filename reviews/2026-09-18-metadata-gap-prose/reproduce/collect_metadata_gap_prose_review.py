...
def main():
    ...
    for name in ('build', 'prior', 'candidate', 'rebuild', 'english', 'conversion-trial', 'readme', 'output'):
    ...
    for language in ('english', 'chinese'):
        ...
        copy(args.prior / 'renders' / (stem + '.docx'), 'before/' + stem + '.docx')
        copy(args.prior / 'word-preview' / (stem + '.pdf'), 'before/' + stem + '-word.pdf')
        ...
        for label, root, key in [('before', args.prior, 'before'), ('after', args.build, 'after')]:
            ...
            geometry = row['word_prose_geometry'][key]
            word_page = geometry[0]['page'] if key == 'before' else geometry['page']
            subprocess.run(['pdftoppm', '-f', str(word_page), '-l', str(word_page), '-r', '105', '-singlefile', '-png',
                str(root / 'word-preview' / (stem + '.pdf')), str(args.output/'visual'/(label+'-'+language+'-word-page'+str(word_page)))], check=True, capture_output=True)
