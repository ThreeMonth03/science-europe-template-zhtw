...
def main():
    ...
    p.add_argument('--suite', choices=['basic', 'mixed-boundary'], default='basic')
    ...
    checker = 'check_header_controls.py' if a.suite == 'basic' else 'check_mixed_boundary_controls.py'
    assert check['checker_sha256'] == sha(ROOT / 'scripts' / checker)
    if a.suite == 'mixed-boundary':
        from prepare_mixed_boundary_controls import CASES
        assert check['planned_position_controls_complete']
    else:
        from prepare_header_controls import CASES
    assert check['cases'] == CASES
    assert {r['stem'] for r in check['rows']} == {c + '-' + p + '-' + l for c in CASES for p in ['review', 'submission'] for l in ['english', 'chinese']}
    ...
    if a.suite == 'mixed-boundary':
        for name in ['prepare_mixed_boundary_controls.py', 'generate_mixed_budget_fixtures.py',
                     'check_mixed_boundary_controls.py', 'mixed_row_content.py', 'mixed_budget_trial.py',
                     'check_word_boundary_content.py']:
            keep(ROOT / 'scripts' / name, 'reproduce/' + name)
    for name in ['prototype.py', 'compact.py', 'finish.py']:
        keep(ROOT / 'experiments/mixed-budget-header' / name, 'reproduce/experiment/' + name)
    for path in a.check.parent.glob(('header-control-' if a.suite == 'basic' else 'mixed-boundary-') + '*.png'):
    ...
    write('inventory.json', dict(prototype_only=True, version='0.3.42', source_repo_modified=False,
        ...
        control_suite=a.suite, collector_sha256=sha(Path(__file__))))
    print(a.output)
