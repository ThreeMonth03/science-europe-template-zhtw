...
def main():
    ...
    p.add_argument('--without-capture', action='store_true', help='A tables-only control run without the mixed-only observer')
    a = p.parse_args()
    ...
    initial = state()
    expected_command = ['dsw-document-worker', 'run'] if a.without_capture else ['python', '-u', '/capture.py']
    assert initial[0]['command'] == expected_command
    ...
    report = {'before': initial, 'deleted_owned_templates': 4, 'production_touched': False,
              'template_zip_backups_retained': True, 'capture_observer_attached': not a.without_capture}
    env = dict(os.environ)
