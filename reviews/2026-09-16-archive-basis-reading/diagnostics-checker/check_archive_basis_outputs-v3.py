...
def word_delta(before,after,old_texts,new_text):
    left,right=body(before),body(after);removed_id=None;style_transition=False
    if old_texts:
        ...
        allowed={None}|{style for n in group for _,style in formatted_characters(n)}
        assert all(style in allowed for _,style in formatted_characters(right[index])), 'New direct character formatting'
    ...
    for position,(a,b) in enumerate(zip(left,right)):
        ...
        # Pandoc treats the first paragraph after a list as FirstParagraph.
        # With the list gone, ONLY its immediate successor becomes BodyText.
        if old_texts and position==index+1 and xml(a)!=xml(b):
            old_style=a.find('./'+qn('w:pPr')+'/'+qn('w:pStyle'))
            new_style=b.find('./'+qn('w:pPr')+'/'+qn('w:pStyle'))
            if old_style is not None and new_style is not None and old_style.get(qn('w:val'))=='FirstParagraph' and new_style.get(qn('w:val'))=='BodyText':
                x,y=[after.styles[n]._element for n in ['First Paragraph','Body Text']]
                assert x.find(qn('w:basedOn')).get(qn('w:val'))=='BodyText'
                for tag in ['w:pPr','w:rPr']:assert xml(x.find(qn(tag)))==xml(y.find(qn(tag)))
                candidate.find('./'+qn('w:pPr')+'/'+qn('w:pStyle')).set(qn('w:val'),'BodyText')
    ...
    try:
        for case in a.cases:
            ...
            for language in ['english','chinese']:
                ...
                compare(*[BeautifulSoup(str(s.select_one('#dmp-content')),'html.parser') for s in [before,after]],language)
