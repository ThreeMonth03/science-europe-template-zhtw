            ...
            for language in ['english', 'chinese']:
                ...
                for old, new in zip(old_soup.select('.question'), soup.select('.question')):
                    ...
                    for prompt in new.select('[data-fact-id="personal-data-safeguards"]'):
                        prompt.decompose()
                    if (name, language, old['id']) == ('personal-data-partial', 'chinese', 'q-personal-data'):
                        # The frozen malformed paragraph swallowed exactly this
                        # source indentation. The final paragraph must be closed
                        # and preserve the entire owned public-interest sentence.
                        old_p = old.select('.answer > p'); new_p = new.select('.answer > p')
                        sentence = ' 本計畫蒐集及處理個人資料，法律依據為公共利益。'
                        assert len(old_p) == len(new_p) == 1
                        assert old_p[0].get_text() == sentence + '\n    \n    '
                        assert new_p[0].get_text() == sentence
                        old_p[0].string = sentence
