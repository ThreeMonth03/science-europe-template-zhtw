...
def verified_list_suffixes(pages, bbox, soup, start=0):
    ...
    for index, (page, node) in enumerate(zip(pages, geometry)):
        if index < start:
            clean.append(page)
            continue
        text = page.rstrip('•')
...
def compare_word(before, after, pairs):
    ...
    for left, right in zip(old, new):
        ...
        assert removed > 0
        properties = lambda node: xml(node) if node is not None else None
        assert properties(left.find(qn('w:pPr'))) == properties(right.find(qn('w:pPr'))), 'Policy style changed'
    ...
    try:
        for case in a.cases:
            ...
            for language in ['english','chinese']:
                ...
                heading = compact(before.select_one('#q-persistent-identifier h3').get_text())
                starts = [next(i for i,p in enumerate(pages) if heading in p) for pages in [old_pages,new_pages]]
                assert starts[0] == starts[1], 'Q13 moved across an unreviewed earlier-page marker boundary'
                clean_old, old_bullets = verified_list_suffixes(old_pages,subprocess.check_output(['pdftotext','-bbox-layout',str(old.with_suffix('.pdf')),'-']),before,starts[0])
                clean_new, new_bullets = verified_list_suffixes(new_pages,subprocess.check_output(['pdftotext','-bbox-layout',str(new.with_suffix('.pdf')),'-']),after,starts[1])
                ...
                if case == 'budget-mixed-gaps':
                    row['policy_line_counts'] = []
                    for index,(old_text,new_text) in enumerate(pairs,1):
                        for kind,old_path,new_path in [('pdf',old.with_suffix('.pdf'),new.with_suffix('.pdf')),
                                ('word',prior/'word-preview'/(stem+'.pdf'),preview)]:
                            left_lines = prompt_lines(old_path,old_text); right_lines = prompt_lines(new_path,new_text)
                            assert (left_lines['line_count'],right_lines['line_count']) == ((2,1) if language=='chinese' else (2,2))
                            row['policy_line_counts'].append({'distribution':index,'format':kind,'before':left_lines,'after':right_lines})
