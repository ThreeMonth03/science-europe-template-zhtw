...
def fonts(pdf):
    rows=subprocess.check_output(['pdffonts',str(pdf)],text=True).splitlines()[2:]
    result=[]
    for row in rows:
        fields=row.split();assert len(fields)>=8
        name=fields[0].split('+',1)[-1]
        result.append([name,*fields[1:-2]])  # subset prefix and object IDs are not font identities
        ...
        for case in a.cases:
            ...
            for profile in ('review','submission'):
                for language in ('english','chinese'):
                    ...
                    font_pair=[fonts(path.with_suffix('.pdf')) for path in (old,new)]
                    assert font_pair[0]==font_pair[1],(stem,'Native font inventory changed',font_pair)
                    ...
                    row=dict(case=case,profile=profile,language=language,selected=selected,question_comparisons=count,
                        ...
                        pdf_font_inventory=font_pair[1],pdf_font_inventory_unchanged=True,
