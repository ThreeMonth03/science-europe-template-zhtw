...
def main():
    ...
    assert all(Path(name).name == name and name.endswith('.html') for name in a.cases)
    ...
    payload = {'sequence': sequence, 'gc_policy': a.gc}
    ...
    completed = [e for e in events if e['event'] == 'render-complete']
    passed = result.returncode == 0 and len(completed) == len(sequence) and events[-1]['event'] == 'complete'
    report = {'diagnostic_only': True, 'release_acceptance': False, 'completed_without_crash': passed,
        ...
        'image': a.image, 'image_id': image_id, 'sequence': sequence, 'completed_renders': len(completed),
    ...
    print(json.dumps({k: report[k] for k in ['completed_without_crash', 'returncode', 'seconds', 'completed_renders', 'gc_policy']}), flush=True)
