...
目前 [0.3.45 閱讀品質整合](docs/submission-reading.md)已把後續原型接回英文來源，中文仍由
既有流程重建。七個共用檔案、中性計畫／資源／軟體編號、第 9 題提示與引導句、
空預算區塊及 Word 短表格保護一起配對升版；原有 762 組譯文保留，只新增五組。
下列「尚未整合」說明是各次原型當時的狀態，保留其歷史，不代表目前來源版本。
已完成[實際新版套件的 12 組原生對照](reviews/2026-09-22-submission-reading-integration/README.md)：
新版與同日舊版各 36 份 HTML／PDF／Word、12 份 Word 預覽，全文及逐頁座標／像素
一致，四張短表完整同頁，中文長文提交 PDF 維持 7 頁。未部署、未全面驗收。

前版 [0.3.44 提交預覽整合](docs/submission-preview.md)：提示切換、概要／品管合段及中性
...
已完成[新版套件 16 組原生對照](reviews/2026-09-21-submission-preview-integration/README.md)：
48 份原生 HTML／PDF／Word 與 16 份 Word 預覽，全文、Word 組件及逐頁座標／影像
重現先前原型。沒有部署，也不是全域提交開關或 Microsoft Word 全面驗收。

後續[計畫／資源／軟體中性名稱原型](reviews/2026-09-21-entity-labels/README.md)完成
1,056 組結構與四組中英原生前後比較；檢核版不變，名稱編號不因段落篩選而漂移。
尚未接回來源或升版。新案例另確認第 9 題仍有未關閉的系統提示，列入下一輪修補。

後續[第 9 題提示原型](reviews/2026-09-21-ethics-prompts/README.md)已通過 536 組結構與
八組原生內容比較，但**中文 PDF 引導句跨頁的排版關卡未過**。暫不整合升版；先修
引導句與第一段回答的銜接，不能只憑頁數與內容保真放行。

後續[第 9 題連頁實驗](reviews/2026-09-21-ethics-lead/README.md)已讓短回答的中文引導句與
首段同頁，544 組結構與 12 組原生內容比較通過；但中文長文提交 PDF 由 7 頁增為 8 頁，
**整合仍被阻擋**。另揭露既有 Word 檢核版的孤立重複表頭，尚未全篇視覺驗收。
三次嘗試與失敗證據均保留，正式英文／中文來源及 0.3.44 版號未變。

後續[空預算區塊與 Word 表格診斷](reviews/2026-09-21-empty-budget/README.md)：提交版只省略
沒有預算項目的重複計畫標題，中文長文 PDF **8 → 7 頁**，已回答案例 **5 → 4 頁**。
816 組結構與 12 組原生內容比較通過；檢核控制不變，名稱、原始編號及已填內容保留。
Word 20 組診斷副本已確認直接列屬性可修短表，但共用 table style 路徑未奏效，
**原生 Word 孤立表頭仍阻擋整合**。未以加工副本冒充模板修正，來源及版本未變。

後續[Word 短表格共用轉換原型](reviews/2026-09-21-word-short-tables/README.md)已由 DSW
原生流程修正上述孤立表頭：中英及檢核／提交 12 組對照通過，四張短表完整同頁，
HTML／PDF 不變，長表控制仍可跨頁。37 組引擎控制及嚴格 XML 還原檢查通過。
但另發現翻譯器會把 XML helper 的七個程式片段當成譯句，**來源整合仍被阻擋**。
相同內容改走 `.xml` 共用資產已通過最小翻譯演練，尚待 DSW 資產載入實測；不可把
兩項獨立成功當成端到端完成。來源、譯文與 0.3.44 版號不變，未部署。

後續[共用 XML 資產與完整翻譯演練](reviews/2026-09-21-word-asset/README.md)已確認
DSW 可載入共用資產：12 組原生 HTML、PDF 與 Word 對照全部不變。完整來源翻譯
另抓到 Q9 句間空格與 Q15 標籤尾端空格回歸，已在英文共用寫法修正，
3,312 組結構比較通過；762 組舊譯文完整保留，僅新增五組。沒有另修中文 Jinja。
尚待接回正式來源、配對升版及實際重建套件的原生驗證；本輪仍未部署或全面驗收。
