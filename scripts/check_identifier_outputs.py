...
def body_pages(pdf):
    ...
    for page in pages:
        ...
        if lines and re.fullmatch(r'\s*\d+/\d+\s*',lines[-1]): lines.pop()
...
def main():
    ...
    report={'selected_checks_passed':False,'release_acceptance':False,'rows':[],
            ...
            'helper_sha256':{n:sha(Path(__file__).with_name(n)) for n in ['check_answer_state_outputs.py','check_context_outputs.py','check_narrative_outputs.py','check_paper_outputs.py','check_preservation_outputs.py','check_word_rhythm_outputs.py','compare_runtime_outputs.py']},
