    ...
    for case in CASES:
        for language, locale in [('english', 'en'), ('chinese', 'zh-Hant')]:
            ...
            for profile in ['review', 'submission']:
                ...
                assert html_sha == receipt['original_html_sha256'] and fonts == receipt['fonts'], (stem, 'Native HTML drift')
                assert raw == old.with_suffix('.html').read_bytes()
...
if __name__ == '__main__':
    ...
    for name in ['after', 'fixtures', 'english', 'output']: p.add_argument('--' + name, type=Path, required=True)
    ...
    try:
        report['rows'] = run(a.after.resolve(), a.fixtures.resolve(), a.english.resolve())
        report.update(passed=True, native_rebuilt_source_checked=True, exact_prototype_parity=True)
