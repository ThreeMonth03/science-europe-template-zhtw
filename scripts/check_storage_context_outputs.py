...
def word_parts(doc):
    ...
    assert {'5','6'} <= headings.keys(), 'Q5/Q6 heading missing or restyled'
...
def check_word(before,after,selected):
    ...
    if selected:
        paragraphs=[[Paragraph(n,d) for n in parts[1] if n.tag==qn('w:p')] for parts,d in [(left,before),(right,after)]]
        assert len(paragraphs[0])==len(paragraphs[1])==5
        old_styles=[p.style.style_id for p in paragraphs[0]]
        assert [p.style.style_id for p in paragraphs[1]]==[old_styles[0],'PilotLead','PilotLead','PilotListLead',old_styles[-1]], 'Only policy, intro and first bullet may gain keep styles'
...
def main():
    ...
    report=dict(selected_checks_passed=False,content_and_scope_checks_passed=False,pagination_checks_passed=False,release_acceptance=False,rows=[],checker_sha256=sha(Path(__file__)),
    ...
    try:
        for case in CASES:
            ...
            for language in ['english','chinese']:
                ...
                if selected:
                    row['q5_locations']={};row['pagination_issues']=[]
                    for kind,files in [('pdf',[old.with_suffix('.pdf'),new.with_suffix('.pdf')]),('word',previews)]:
                        ...
                        together=len(set(right_pages))==1
                        if not together:row['pagination_issues'].append(dict(format=kind,locations=right_pages))
                        row['q5_locations'][kind]={'before':left_pages,'after':right_pages,'fixed_separation':len(set(left_pages))>1 and together}
                else:
                    row['pagination_issues']=[]
                ...
                row.update(eligible=selected,content_and_scope_checks_passed=True,passed=not row['pagination_issues'],prior_root=str(prior))
                ...
                print(json.dumps({k:row[k] for k in ['case','language','eligible','pages','word_pages','word_style_changes','pagination_issues']}),flush=True)
        report['content_and_scope_checks_passed']=True
        report['pagination_checks_passed']=all(r['passed'] for r in report['rows'])
        report['selected_checks_passed']=report['pagination_checks_passed']
    ...
    if not report['selected_checks_passed']:raise SystemExit('FAIL: unresolved native pagination; see '+str(target))
