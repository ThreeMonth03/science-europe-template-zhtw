"""Q3 metadata-only 0.3.33 source/translation delta; retain Q2 and Q11 behavior gates."""
import argparse
import io
import json
...
from probe_personal_data_translation import verify_metadata_translation_chain
...
def main():
    ...
    personal, following = verify_metadata_translation_chain([pair(f.read_text()) for f in files])
    ...
    baseline = following['metadata_followup']['baseline']
    ...
    assert changed and all(n.startswith('translation/tree/src/questions/03-docs-metadata.html.j2/') for n in changed)
    prior = ROOT/'reviews/2026-09-17-storage-capacity-gaps/probes/storage-gap-scope.json'
    ...
    for old_row in proof['rows']:
        ...
        assert differences == ['src/questions/03-docs-metadata.html.j2', 'src/uuids.j2'], differences
        baseline_ids = subprocess.check_output(['git', '-C', str(a.english), 'show', '4f8546a363574a0eda0a8df2a6eee05df67029a2:src/uuids.j2'], text=True)
        additions = ''.join('{%- set '+n+' = "'+v+'" -%}\n' for n, v in [
            ('metadataDictionaryQUuid', '434b28c1-dbc0-4dd0-bb53-6d6ad51b86f4'),
            ('metadataDictionaryNoAUuid', '93d19bdb-a977-42d6-9ecd-edfaed543281'),
            ('metadataDictionaryYesAUuid', 'c6efc94b-68e0-4d1c-a8c9-f02bd736623c')])
        lines = baseline_ids.splitlines(keepends=True)
        index = next(i for i, line in enumerate(lines) if line.startswith('{%- set metadataExploreAUuid ='))+1
        assert (root/'src/uuids.j2').read_text() == ''.join(lines[:index])+additions+''.join(lines[index:]), 'Unreviewed UUID file change'
        ...
        checks.append({'language': folder, 'before': before, 'after': after, 'changed': differences,
            'q3_exact_dom_checks': metadata_count, 'q3_retained_capacity_checks': count, 'q2_exact_dom_checks': q2, 'q11_branch_checks': q11,
    ...
    report = {'passed': True, 'release_acceptance': False, 'rows': checks, 'reviewed_deltas': following,
        ...
        'contract_sha256': sha(a.english/'scripts/metadata_followup_contract.py'),
        'package_sha256': {n: sha(a.build/n) for n in ['english.zip', 'chinese.zip']}}
    target = a.build/'metadata-followup-scope.json'; assert not target.exists()
    target.write_text(json.dumps(report, ensure_ascii=False, indent=2)+'\n')
    print(json.dumps({'passed': True, 'q3_checks': sum(r['q3_exact_dom_checks'] for r in checks), 'units': len(files)}))
