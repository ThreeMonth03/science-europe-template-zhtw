...
def word_delta(before,after,pairs):
    ...
    for a,b in zip(left,right):
        ...
        for op,i,j,k,l in difflib.SequenceMatcher(a=p.text,b=q.text,autojunk=False).get_opcodes():
            ...
            else:assert all(style is None for _,style in x[i:j]+y[k:l]), 'Changed styled text'
        changed+=1
    ...
    try:
        for case in CASES:
            for language in ['english','chinese']:
                ...
                for f in [new.with_suffix('.pdf'),preview]:
                    bbox=subprocess.check_output(['pdftotext','-bbox-layout',str(f),'-'])
                    assert not line_box_overlaps(bbox), 'PDF/Word overlapping lines'
                if not pairs:
