...
def main():
    ...
    parser.add_argument('--review-document', type=Path, default=ROOT / 'docs/storage-sharing-review.md')
    parser.add_argument('--extra-cases', nargs='*', default=[], help='Additional Chinese PDF/DOCX cases to archive')
    parser.add_argument('--checked-report', nargs=2, action='append', default=[], metavar=('REPORT', 'CHECKER'),
                        help='Additional build-relative report and repo-relative checker, verified before copying')
    ...
    for side, root in [('stock', args.baseline), ('tables', args.variant)]:
        ...
        for name, checker in args.checked_report:
            extra = json.loads((root / name).read_text())
            assert extra['selected_checks_passed'] and extra['release_acceptance'] is False
            assert extra['checker_sha256'] == sha(ROOT / checker)
            for artifact, expected in extra['sha256'].items(): assert sha(root / artifact) == expected
    args.destination.mkdir(parents=True, exist_ok=False)
    for side, root in [('stock', args.baseline), ('tables', args.variant)]:
        out = args.destination / side; out.mkdir()
        selections = [('structured', 'chinese'), ('storage-sharing', 'chinese'), ('storage-sharing-partial', 'chinese'), ('storage-sharing', 'english')]
        selections += [(case, 'chinese') for case in args.extra_cases]
        for case, language in selections:
            for fmt in ('pdf', 'docx'):
        ...
        for name, _ in args.checked_report:
            shutil.copy2(root / name, out / name)
