"""Q5-only bounded layout delta; retain Q2/Q3/Q11 behavior and all 744 translations."""
import argparse
import io
import json
...
def main():
    ...
    baseline = '394507ec1032bbd085be23ae57e5f07ffde42199'
    ...
    assert not changed, 'Translation files must remain byte-identical'
    prior = ROOT/'reviews/2026-09-17-metadata-followups/probes/metadata-followup-scope.json'
    ...
    for old_row in proof['rows']:
        ...
        assert set(after)-set(before) == {'src/storage-reading.html.j2'} and not set(before)-set(after)
        ...
        assert sha(context_frozen) == before['src/questions/05-store-backup.html.j2']
        ...
        checks.append({'language': folder, 'before': before, 'after': after, 'changed': differences,
            'q5_checks': len(context_rows), 'q5_eligible': sum(r['eligible'] for r in context_rows), 'q3_exact_dom_checks': metadata_count, 'q3_retained_capacity_checks': count, 'q2_exact_dom_checks': q2, 'q11_branch_checks': q11,
    ...
    report = {'passed': True, 'release_acceptance': False, 'rows': checks, 'reviewed_deltas': following,
        ...
        'contract_sha256': sha(a.english/'scripts/storage_context_contract.py'),
        'package_sha256': {n: sha(a.build/n) for n in ['english.zip', 'chinese.zip']}}
    target = a.build/'storage-context-scope.json'; assert not target.exists()
