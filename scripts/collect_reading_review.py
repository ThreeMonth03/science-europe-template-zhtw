"""Archive reading-unit experiment evidence; never publish/stage a release."""
import argparse
import json
import shutil
...
def main():
    ...
    for side, root in [('stock', args.baseline), ('tables', args.variant)]:
        ...
        assert pilot['checker_sha256'] == sha(ROOT / 'scripts/run_pilot.py')
        ...
        for name, script in [('reading-report.json', 'check_reading_outputs.py'), ('quality-report.json', 'check_quality_outputs.py')]:
    ...
    for name, script in [('quality-translation-probe.json', 'probe_quality_translation.py'), ('reading-translation-probe.json', 'probe_reading_translation.py')]:
        ...
        if name == 'quality-translation-probe.json': assert probe['assertion_helper_sha256'] == sha(ROOT / 'scripts/check_quality_outputs.py')
    args.destination.mkdir(parents=True, exist_ok=False)
    for side, root in [('stock', args.baseline), ('tables', args.variant)]:
        out = args.destination / side; out.mkdir()
        cases = [('structured', 'chinese'), ('reading-rich', 'chinese'), ('reading-partial', 'chinese'), ('table-long', 'chinese'), ('structured', 'english')]
        if side == 'stock': cases += [('empty', 'chinese')]
        checked = {(row['case'], row['language']) for row in json.loads((root / 'reading-report.json').read_text())['rows']}
        ...
        for name in ('manifest.json', 'pilot-report.json', 'render-results.json', 'reading-report.json', 'quality-report.json'):
            shutil.copy2(root / name, out / name)
        shutil.copytree(root / 'word-preview', out / 'word-preview')
    for name in ('km-binding-audit.json', 'translation-audit.json', 'structure-audit.json', 'quality-translation-probe.json', 'reading-translation-probe.json'):
