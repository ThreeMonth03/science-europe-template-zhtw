...
from probe_personal_data_translation import archived_pairs, verify_latest_translation_chain, verify_prose_translation_chain
...
class StorageGapDeltaTests(unittest.TestCase):
    def test_current_tree_exact_reviewed_chain(self):
        ...
        personal, following = verify_prose_translation_chain(values)
        ...
        for index in [0, -1, -2]:
            ...
            with self.assertRaises(AssertionError): verify_prose_translation_chain(mutated)
        with self.assertRaises(AssertionError): verify_prose_translation_chain(values[:-1])
