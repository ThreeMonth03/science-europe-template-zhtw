...
from probe_personal_data_translation import archived_pairs, verify_prose_translation_chain
...
class MetadataGapProseDelta(unittest.TestCase):
    def test_exact_current_tree_retains_every_previous_pair(self):
        ...
        _, following = verify_prose_translation_chain(current)
        self.assertEqual(following['metadata_gap_prose']['retained_units'], 744)
        self.assertEqual(len(current), 747)
